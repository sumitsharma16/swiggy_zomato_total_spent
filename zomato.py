...
def get_dish_name(dish_names):
    dishes = dish_names.split(",")
    dish_names = []
    for dish in dishes:
        dish_name = dish.split(' x ')[-1]
        dish_names.append(dish_name)
    return dish_names

def getOrders(cookies):
    print("Retrieving Zomato...")
    ...
    while True:
        ...
        # total_page = resp['sections']['SECTION_USER_ORDER_HISTORY']
        # print(total_page)
        ...
        for order_id in resp['entities']['ORDER']:
            ...
            num_of_orders += 1
            spent += order_total
            dish_string = order['dishString']
            dish_names = get_dish_name(dish_string)
            for dish in dish_names:
                cnt[dish] += 1
                if dish.lower().contains('biryani'):
                    biryaniCount+=1
            resName = order["resInfo"]["name"]
            res[resName]+=1
    ...
    resName = order["resInfo"]["name"]
    last_order_date = order["orderDate"]
    
    print("Biryani Count: ",biryaniCount)
    
    print("Last order ID: ",last_order)
    print("Order Date: ",last_order_date)
    print("Restaurant Name: ",resName)
    print("Dish: ",dish)
    print("Dish Count: ",sorted(cnt.items(),key=lambda x:x[1],reverse=True))
    print("Restaurant Count: ",sorted(res.items(),key=lambda x:x[1],reverse=True))

    average_spent = spent//num_of_orders 
    print()
    print(
        f"[red]Total money spent on zomato.com : [bold]INR {spent:,}[/bold][/red]")
    print(
        f"[green]Total number of orders placed : [bold]{num_of_orders:,}[/bold][/green]")
    print(
        f"[yellow]Average money spent on each order : [bold]INR {average_spent:,}[/bold][/yellow]")
